...
function SortableSlideCard({ id, img, index, onDelete, onUpdate, isUploading }: SortableSlideCardProps) {
  ...
  const handleDesktopFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => { onUpdate(index, 'url', reader.result as string); };
  ...
  const handleMobileFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    const reader = new FileReader();
    reader.onloadend = () => { onUpdate(index, 'mobile_image', reader.result as string); };
    reader.readAsDataURL(file);
    e.target.value = '';
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-12 gap-6">
          ...
          <div className="flex flex-wrap gap-1 bg-white p-1 rounded-xl shadow-sm border border-zinc-200">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg font-bold uppercase tracking-wider text-[11px] transition-all whitespace-nowrap ${activeTab === 'blog' ? 'bg-zinc-900 text-white shadow-md' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50'}`}
            >
                    ...
                    {filteredProducts.length === 0 && !isLoading ? (
                      ...
                      paginatedProducts.map(product => (
                        ...
                        <div className="p-3 sm:p-6 flex items-center gap-3 sm:gap-6 hover:bg-zinc-50 transition-colors group border-b border-zinc-100">
                          ...
                          {/* Expand variants */}
                          <button
                            onClick={() => { toggleExpand(product.id); fetchVariantsForProduct(product.id); }}
                          ...
                          <div className="bg-zinc-50 border-b border-zinc-100 px-4 py-3 pl-6 sm:pl-[calc(1.5rem+44px+80px+1.5rem)]">
                            {variantCache[product.id] ? (
                              variantCache[product.id].length === 0 ? (
                                ...
                                <div className="space-y-2">
                                  {Object.entries(
                                    variantCache[product.id].reduce<Record<string, any[]>>((acc, v) => {
                                      const key = v.color || '';
                                      if (!acc[key]) acc[key] = [];
                                      acc[key].push(v);
                                      return acc;
