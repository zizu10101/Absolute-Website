...
- **Session 76:** ProductCard.tsx — hide color thumbnails when product has only one non-default color. Changed condition from `product.colors.length > 0` to `product.colors.filter(c => !c.isDefault).length > 1` so the thumbnail row (including the default/master button) only appears when there are 2+ actual color choices.
