...
- **Session 73:** ProductCard.tsx image + color-swatch fixes (applies everywhere ProductCard is used: main grid, HomePage New Arrivals/On Sale, ProductDetailPage related products). (1) Image container changed from `aspect-[4/5]`/`bg-white` to `aspect-square`/`bg-[#f6f6f6]`/`rounded-lg`, and the `<img>` gained `object-center` alongside its existing `object-contain` — makes every card the same height regardless of source image dimensions, with a light gray letterbox instead of white. (2) Color swatch row was rendering even for single-color products because the outer gate only checked raw `product.colors.length > 0`, while the actual swatches rendered are `product.colors` minus the `isDefault`-flagged entry — a product with just that one default entry showed an empty-looking swatch row (just the redundant "Default" button). Fixed by hoisting that filter into a `nonDefaultColors` variable and gating the whole block on `nonDefaultColors.length > 0`, so swatches only appear when a product actually has 2+ distinct colors. Not visually verified — see Pending Items.
